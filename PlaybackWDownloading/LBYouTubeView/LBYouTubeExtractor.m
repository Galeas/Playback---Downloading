...
NSString* const kSUYouTubePlayerExtractorErrorDomain = @"LBYouTubeExtractorErrorDomain";
        ...
        if (decodingError) {
            *error = decodingError;
            ...
            if (streamURL) {
                ...
                *error = [NSError errorWithDomain:kSUYouTubePlayerExtractorErrorDomain code:2 userInfo:[NSDictionary dictionaryWithObject:@"Couldn't find the stream URL." forKey:NSLocalizedDescriptionKey]];
            }
        }
    }
    else {
        *error = [NSError errorWithDomain:kSUYouTubePlayerExtractorErrorDomain code:3 userInfo:[NSDictionary dictionaryWithObject:@"The JSON data could not be found." forKey:NSLocalizedDescriptionKey]];
    }
...
-(void)connectionDidFinishLoading:(NSURLConnection *) connection {
    ...
    if (html.length <= 0) {
        [self _failedExtractingYouTubeURLWithError:[NSError errorWithDomain:kSUYouTubePlayerExtractorErrorDomain code:1 userInfo:[NSDictionary dictionaryWithObject:@"Couldn't download the HTML source code. URL might be invalid." forKey:NSLocalizedDescriptionKey]]];
        return;
    }
